...
export default function FlightPlansTable({
  ...
  const satelliteMap = useMemo(
    () => new Map(satellites.map((sat) => [sat.id, sat.name])),
    [satellites]
  );
  const groundStationMap = useMemo(
    ...
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {row.getValue("groundStationName")} (ID: {plan.gsId})
                </p>
              </TooltipContent>
            </Tooltip>
          );
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              <FlightStatusBadge status={plan.status as FlightPlanStatus} />
              {plan.approverName &&
  ...
  const filteredData = useMemo(() => {
    ...
    const selectedSatellites =
      searchParams.get("satellites")?.split(",").filter(Boolean) || [];
    const selectedGroundStations =
      searchParams.get("groundStations")?.split(",").filter(Boolean) || [];
      ...
      filtered = filtered.filter((plan) => {
        ...
        switch (selectedTimeRange) {
          ...
          case "tomorrow":
            ...
            return (
              scheduledDate >= tomorrowStart && scheduledDate < dayAfterTomorrow
            );
          ...
          case "next-week":
            ...
            return (
              scheduledDate >= nextWeekStart && scheduledDate < nextWeekEnd
            );
          ...
          case "next-month":
            ...
            return (
              scheduledDate >= nextMonthStart && scheduledDate < nextMonthEnd
            );
