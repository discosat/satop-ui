...
export function SearchForm({
  ...
  const currentSatellites =
    searchParams.get("satellites")?.split(",").filter(Boolean) || [];
  const currentGroundStations =
  ...
  const satelliteCounts = React.useMemo(() => {
    ...
    satellites.forEach((sat) => {
      const count = flightPlans.filter(
        (plan) =>
          plan.satId.toString() === sat.id.toString() &&
              ...
              <CommandGroup className="max-h-64 overflow-auto">
                {satellites.map((satellite) => (
                  <CommandItem
                    key={satellite.id}
                    onSelect={() =>
                      handleSatelliteFilter(satellite.id.toString())
                    }
              ...
              <CommandGroup className="max-h-64 overflow-auto">
                {groundStations.map((groundStation) => (
                  <CommandItem
                    ...
                    onSelect={() =>
                      handleGroundStationFilter(groundStation.id.toString())
                    }
                    className="flex items-center justify-between"
                  >
                    <div className="flex items-center">
                      <div
                        className={cn(
                          "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                          currentGroundStations.includes(
                    ...
                    <Badge variant="secondary" className="ml-2">
                      {groundStationCounts.get(groundStation.id.toString()) ||
                        0}
                    </Badge>
                  </CommandItem>
                ))}
